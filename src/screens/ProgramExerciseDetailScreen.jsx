...
const ProgramExerciseDetailScreen = ({ exercise, onBack, customization, onVideoCompleted }) => {
  ...
  const onFullscreen = () => {
    ...
    if (!paused) scheduleHide();
    if (Orientation) {
      try { Orientation.lockToLandscape(); } catch (error) {
  ...
  const onExitFullscreen = () => {
    ...
    if (!paused) scheduleHide();
    if (Orientation) {
      try { Orientation.lockToPortrait(); } catch (error) {
      ...
      <ScrollView style={styles.panel} showsVerticalScrollIndicator={false}>
        <View style={styles.titleRow}>
          ...
          <Text style={styles.difficulty}>
            {Array.isArray(exercise?.level) ? exercise.level.join(', ') : (exercise?.level || 'Easy')}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  ctrlOverlayFS: { paddingHorizontal: 32 },
  ctrlRow: {
