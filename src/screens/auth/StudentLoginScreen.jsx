...
const StudentLoginScreen = ({ onBack }) => {
  ...
  const [generatedOtp, setGeneratedOtp] = useState(null); // shown inline
  ...
  const handleOtpLogin = async () => {
    ...
    setLoading(true);
    try {
      // Get real JWT token from backend using phone number
      const response = await ApiService.post('/auth/login-phone', { phone });

      if (response?.status === 'success' && response?.data?.token) {
        const AsyncStorage = require('@react-native-async-storage/async-storage').default;
        await AsyncStorage.setItem('loginSource', 'student');
      ...
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false} bounces={false}>
          ...
          <Animated.View style={[styles.logoSection, { opacity: fadeAnim, transform: [{ scale: logoScale }, { translateY: slideAnim }] }]}>
            ...
            <Text style={styles.welcomeSubtitle}>Sign in to continue your educational journey</Text>
          </Animated.View>
...
const styles = StyleSheet.create({
  ...
  backgroundPattern: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(244, 240, 240, 1)' },
  ...
  welcomeTitle: { fontSize: 26, fontWeight: '800', color: '#1f2937', textAlign: 'center', marginBottom: 6 },
  welcomeSubtitle: { fontSize: 15, color: '#2d3035', textAlign: 'center', lineHeight: 20 },
  formContainer: {
    backgroundColor: '#fff', borderRadius: 20, padding: 24, marginHorizontal: 4,
